...
def Init():
    ...
    item1 = ama.UMSItem()
    item1.local_ip = "10.240.64.10"
    item1.server_ip = "10.16.131.47"
    item1.server_port = 4321
...
def SubscriptionCategory():
    ...
    item.category_type = ama.SubscribeCategoryType.kNone
    item.security_code = "60001"
...
def GetETFCodeList():
    ...
    if (ama.IAMDApi.GetETFCodeTableList(etf_list ,etf_items, 2) == False):
        ama.Tools.DestroyETFItem(etf_item)
        return
    #循环遍历输出ETF代码表信息
