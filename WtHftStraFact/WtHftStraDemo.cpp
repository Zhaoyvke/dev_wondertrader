...
void WtHftStraDemo::on_init(IHftStraCtx* ctx)
{
	...
	_ctx = ctx;
	_ctx->stra_log_info("回调策略on_init");
}
...
{	
	...
	//return;
	/* 以下为源代码，上为更新策略
	_ctx->stra_log_info("回调策略on_tick");
...
{
	_ctx->stra_log_info("策略回调 on_trade");//测试策略
...
{
	_ctx->stra_log_info("策略回调 on_position");//测试策略
}
